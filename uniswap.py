...
class Uniswap:
    ...
    def exchange_address_from_token(self, token_addr: AddressLike) -> AddressLike:
        ...
        print(f'{token_addr} Exchange address  = {ex_addr}')
    ...
    def estimate_gas(self, uni_v2_router_address, self_address, value):
        uni_v2_router_address = self.w3.toChecksumAddress(uni_v2_router_address)
        self_address = self.w3.toChecksumAddress(self_address)
        transaction = {'to': uni_v2_router_address, 'from': self_address, 'value': self._convert_eth_to_Wei(value)}
    ...
    def get_token_price_in_eth(self, owner, weth_address,token) -> Wei:
        ...
        owner = self.w3.toChecksumAddress(owner)#Uni:AMPL
        ...
        token_address = self.w3.toChecksumAddress(token)        
        ...
        return float(weth_balance/token_balance)
   
    def convert_to_eth(self, wei):
        return float(self.w3.fromWei(wei,'ether'))

    def convert_to_token(self, wei):
        gwei = 1000000000
        return  float(wei / gwei)

    def _convert_eth_to_Wei(self, eth_qty) -> Wei:
        return  self.w3.toWei(eth_qty, "ether")
    ...
    def get_eth_token_input_price(self, token: AddressLike, qty: Wei) -> Wei:
        ...
        #qty = self._convert_eth_to_Wei(qty)
        if self.version == 1:
    ...
    def get_ampl_wallet_balance(self, token: AddressLike) -> int:
        """ GEt AMPL wallet balance"""
        _validate_address(token)
        erc20 = self.erc20_contract(token)
        balance: int = erc20.functions.balanceOf(self.address).call()
        gwei = 1000000000
        return int(balance / gwei)


    def get_eth_wallet_balance(self):
        """Get the balance of ETH in a wallet."""
        eth_in_wei = self.w3.eth.getBalance(self.address)
        return float(self.w3.fromWei(eth_in_wei,'ether'))
    ...
    def get_exchange_rate(self, token: AddressLike) -> float:
        ...
        in_eth =  self.w3.fromWei(eth_reserve, 'ether')
        print (f'Eth reserve in Eth = {in_eth}')
        print (f'ETH Reserve = {eth_reserve}')
        token_reserve = self.get_ex_token_balance(token)
        token_in_eth = self.w3.fromWei(token_reserve, 'ether')
        print (f'Token reserve in ETH = {token_in_eth}')
        print(f'Token = {token} Reserve = {token_reserve}')
        return float(in_eth / token_in_eth)
    ...
    def make_trade(
        ...
        if input_token == ETH_ADDRESS:
            qty = self._convert_eth_to_Wei(qty)
            self.write('make_trade: Eth qty in wei ={qty}')
    ...
    def make_trade_output(
        ...
        if input_token == ETH_ADDRESS:
            ...
            print (f'ETH balance in wei = {balance}')
            need = self.get_eth_token_output_price(output_token, qty)
            print (f'Out_put token balance in wei {need}')
            if balance < need:
    ...
    def _eth_to_token_swap_input(
        ...
        if self.version == 1:
            ...
            self.write(f'_eth_to_token_swap_input: Min amount that should be received after considering the slippage percentage {self.max_slippage} = {amount_out_min}')
    ...
    def _token_to_eth_swap_input(
        ...
        if self.version == 1:
            ...
            if recipient is None:
                recipient = self.address
            slippage_ampount = int((1 - self.max_slippage) * cost)
            return self._build_and_send_tx(
    ...
    def _eth_to_token_swap_output(
        ...
        if self.version == 1:
            ...
            print (f'Price of output toen in ETH = {eth_qty}')
    ...
    def _build_and_send_tx(
        ...
        if not tx_params:
            tx_params = self._get_tx_params()
        self.write(f'_build_and_send_tx:Tx = {tx_params}')
        transaction = function.buildTransaction(tx_params)
        #######################################################3
        #gas = function.estimateGas()
        ...
        self.write('_build_and_send_tx: Sending Transaction to Blockchain')
        tx_hash = None
        try:
            tx_bytes = self.w3.eth.sendRawTransaction(signed_txn.rawTransaction)
            tx_hash = tx_bytes.hex()
            self.write(f'_build_and_send_tx:Hash = { tx_hash}')
        finally:
            logger.debug(f"nonce: {tx_params['nonce']}")
            self.last_nonce = Nonce(tx_params["nonce"] + 1)
            self.write(f'_build_and_send_tx:Finally block executed, returning Hash = {tx_hash}')
            return tx_hash

    def get_hash_receipt(self,tx_hash):
        receipt = None
        self.write(f'get_hash_receipt(uniswap.py): Hash = { tx_hash}')
        try:
            self.write(f'get_hash_receipt(uniswap.py): Waiting for Transaction Receipt ')
            self.w3.eth.waitForTransactionReceipt(tx_hash,60)
        except self.w3.exceptions.TimeExhausted:
            self.write(f'get_hash_receipt: TimeExhausted Exception raised, Tiem waited = 60 sec')
            return None
        try:
            self.write(f'get_hash_receipt: Calling getTransactionReceipt') 
            receipt = self.w3.eth.getTransactionReceipt(tx_hash)
        except self.w3.exceptions.TransactionNotFound:
            self.write(f'get_hash_receipt(uniswap.py):Exception raised by waitForTransactionReceipt: TransactionNotFound')
            return None
        #self.write(f'get_hash_receipt: Receipt for Tx: {tx_hash}')
        #self.write(f'get_hash_receipt: Recipt = {receipt}')
        return receipt 
